...
if (e.code === 'Escape' && this.gameState === 'playing') {
        ...
        while (selectedPositions.length < 8) {
            ...
            if (!selectedPositions.find(p => p.id === randomItem.id)) {
                selectedPositions.push({
                    ...randomItem,
                    requiredQuantity: Math.floor(Math.random()*3)+7,
                    pickedQuantity: 0,
                    completed: false
                });
            }
    ...
    pickItem(item) {
        ...
        if (orderItem && !orderItem.completed) {
            if (orderItem.pickedQuantity < orderItem.requiredQuantity) {
                if (item.quantity <= 0) { this.totalErrors++; this.score -= 5; this.updateHUD(); return; } item.quantity--; orderItem.pickedQuantity++;
                this.score += 10;
        ...
        this.updateOrderPanel(); this.updateHUD(); if (this.inShelfView) { this.renderShelfView(); } }
    ...
    returnItem(item) {
        ...
        if (orderItem && orderItem.pickedQuantity > 0) {
            ...
            item.quantity++; if (this.inShelfView) { this.renderShelfView(); }}
    ...
    renderShelfView() {
        ...
        this.ctx.fillStyle = 'white';
        this.ctx.font = '16px Arial';
        this.ctx.fillText('Выход (E/Space)', this.warehouse.width - 95, 40);
